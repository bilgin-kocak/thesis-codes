...
Fe = variable(Fe)
added_term = pow(1 + omega*(c-c00), -2/3)*inner(F.T*F,I - outer(gradc,gradc))
added_term_dgradc = diff(added_term, gradc)
added_term_dFe = diff(added_term, Fe)
added_term_dc = diff(added_term, c)


dpsi_el_dFe = gamma*(Fe - pow(det(Fe), -beta)*inv(Fe).T) #+ added_term_dFe/100000
dpsi_dF = pow(Jc, -1/3)*dpsi_el_dFe
p = -1/3*pow(Jc, -1/3)*inner(dpsi_el_dFe, F)
dpsi_dc = A*ln(c/(1-c)) + B*(1 - 2*c) + omega*p/Jc #+ added_term_dc/100000
dpsi_dgradc = epsilon*grad(c) #+ added_term_dgradc/100000
